...
pph() {
  ...
  echo "//"
  echo "// Copyright (c) 2016-2020 Kris Jusiak (kris at jusiak dot net)"
  echo "//"
  ...
  echo "#error \"[Boost::ext].SML requires C++14 support (Clang-3.4+, GCC-5.1+, MSVC-2015+)\""
  echo "#else"
  echo "#define BOOST_SML_VERSION ${version}'${revision}'${patch}"
  echo "#define BOOST_SML_NAMESPACE_BEGIN namespace boost { inline namespace ext { namespace sml { inline namespace v${version}_${revision}_${patch} {"
  echo "#define BOOST_SML_NAMESPACE_END }}}}"
  echo "#define __BOOST_SML_HAS_BUILTIN(...) __BOOST_SML_HAS_BUILTIN##__VA_ARGS__"
  echo "#if defined(__clang__)"
  echo "#define __BOOST_SML_HAS_BUILTIN__make_integer_seq 1"
  echo "#define __BOOST_SML_HAS_BUILTIN__is_same 1"
  echo "#define __BOOST_SML_UNUSED __attribute__((unused))"
  echo "#define __BOOST_SML_VT_INIT {}"
  echo "#if !defined(BOOST_SML_CFG_DISABLE_MIN_SIZE)"
  ...
  echo "#define __BOOST_SML_HAS_BUILTIN__make_integer_seq 0"
  echo "#define __BOOST_SML_HAS_BUILTIN__is_same 0"
  echo "#define __BOOST_SML_UNUSED __attribute__((unused))"
  echo "#define __BOOST_SML_VT_INIT {}"
  echo "#if !defined(BOOST_SML_CFG_DISABLE_MIN_SIZE)"
  echo "#define __BOOST_SML_ZERO_SIZE_ARRAY(...) __VA_ARGS__ _[0]"
  ...
  echo "#if defined(__GNUC__) && (__GNUC__ >= 10)"
  ...
  echo "#endif"
  echo "#if defined(__BOOST_SML_HAS_BUILTIN)"
  echo "#undef __BOOST_SML_HAS_BUILTIN"
  echo "#undef __BOOST_SML_HAS_BUILTIN__make_integer_seq"
  echo "#undef __BOOST_SML_HAS_BUILTIN__is_same"
  echo "#endif"
  echo "#endif"
  echo "#endif"
  echo
