...
const char *get_type_name() {
  ...
  template <class TEvent>
  using get_event =
      aux::conditional_t<aux::is_base_of<internal_event, TEvent>::value, aux::type_list<>, aux::type_list<TEvent>>;
  ...
  using get_history_states =
      aux::join_t<typename aux::conditional<!Ts::history && Ts::initial, aux::type_list<typename Ts::src_state>,
                                            aux::type_list<>>::type...>;
  template <class>
...
#define BOOST_SML_DISABLE_EXCEPTIONS false
  ...
  struct sm_impl : aux::conditional_t<aux::is_empty<typename TSM::sm>::value, aux::none_type, typename TSM::sm> {
    ...
    using events_t =
        aux::apply_t<aux::unique_t, aux::join_t<sub_internal_events_t, aux::apply_t<get_all_events, transitions_t>>>;
...
#else
      return process_event_except_imp<get_event_mapping_t<get_generic_t<TEvent>, mappings>>(event, deps, subs,
                                                                                            has_exceptions{});
#endif
    }
    template <class TEvent, class TDeps, class TSubs,
  ...
  class sm {
   ...
   private:
    ...
    using deps_t = aux::apply_t<
        aux::pool, aux::apply_t<aux::unique_t, aux::join_t<deps, sm_all_t, logger_dep_t, aux::apply_t<merge_deps, sub_sms_t>>>>;
    struct events_ids : aux::apply_t<aux::inherit, events> {};

   public:
    sm() : deps_{aux::init{}, aux::pool<>{}}, sub_sms_{aux::pool<>{}} {
...
#if defined(__cpp_fold_expressions)
      return (
          call<TEvent, args_t<Ts, TEvent>, typename TSM::logger_t>::execute(aux::get_by_id<Ns>(&g), event, sm, deps, subs) &&
          ...);
#else
      auto result = true;
  ...
  template <class T>
  auto transitional_impl(T &&t) -> aux::always<typename T::dst_state, typename T::src_state, typename T::event,
